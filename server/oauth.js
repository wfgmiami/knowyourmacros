...
module.exports = (app) => {
  ...
  // Google strategy
  passport.use(new GoogleStrategy(app.get('oauth').googleInfo, (token, refreshToken, profile, done) => {
    const nameArray = profile
      .displayName
      .split(' ');
    User.findOrCreate({
      where: {
        ...
        topBadges: [
          {
            ...
            shareImage640px: 'https://static0.fitbit.com/images/badges_new/386px/shareLocalized/en_US/badge_da' +
              'ily_steps5k.png',
  ...
  // Fitbit strategy
  passport.use(new FitbitStrategy(app.get('oauth').fitbitInfo, (token, refreshToken, profile, done) => {
    User
  ...
  app.get('/api/auth/fitbit', (req, res, next) => {
    if (req.headers.host !== 'localhost:3000') {
      res.cookie('token', 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6M30.hzfEQOcJdvat6x_di6IDwi_tykCfKIAeehBukstyS14');
  ...
  app.get(app.get('oauth').fitbitInfo.callbackURL, passport.authenticate('fitbit', {
    failureRedirect: '/',
    session: false
  }), (req, res, next) => {
